...
export default function MoviesManagement() {
    ...
    const handleDeleteOne = (maCumRap) => {
        usersApi.getDanhSachCumRap(maCumRap).then((result) => {
            setData(result.data);
            ...
            if (flag) {
                ...
                alert("Cụm rạp không thể xóa vì đang có lịch chiếu và vé chưa xác thực!!!");
            }
        });
    };
    ...
    const onUpdate = (movieObj) => {
        ...
        setOpenModal(false);
        theatersApi.suaCumRap(movieObj)
        window.location.reload();
        return undefined;
    }
        ...
        <div style={{ height: "100vh", width: "100%", paddingBottom: '150px' }}>
            ...
            <Dialog open={openModal}>
                <DialogTitle onClose={() => setOpenModal(false)}>
                    {selectedPhim?.current?.tenCumRap
                        ? `Sửa Cụm Rạp: ${selectedPhim?.current?.tenCumRap}`
                        : "Thêm Cụm Rạp"}
                </DialogTitle>
