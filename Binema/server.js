...
app.get('/api/ThongKe/getPhim', function (req, res) {
    dbConn.query('SELECT tenPhim, COUNT(*) AS soLuong, SUM(amount) AS tongDoanhThuPhim from nodejsapi.thongke GROUP BY tenPhim', [], function (error, results, fields) {
        if (error) throw error;
        return res.send(results);
    });
});
app.get('/api/ThongKe/getCumRap', function (req, res) {
    dbConn.query('SELECT tenCumRap, COUNT(time) AS suatchieu, COUNT(*) AS soLuongRap, SUM(amount) AS tongDoanhThuRap FROM nodejsapi.thongke GROUP BY tenCumRap', [], function (error, results, fields) {
        if (error) throw error;
        return res.send(results);
    });
});
...
app.get('/api/QuanLyRap/LayThongTinLichChieuPhim', function (req, res) {
    dbConn.query('SELECT * FROM phiminsert JOIN hethongrapvaphim ON phiminsert.maPhim = hethongrapvaphim.maPhim JOIN hethongrap ON hethongrap.hid = hethongrapvaphim.maHeThongRap WHERE phiminsert.maPhim = ?', [req.query.MaPhim], async (error, results0, fields) => {
        ...
        const final = {
            ...
            "tenTheLoaiPhim": results0[0].tenTheLoai,
...
app.post('/api/QuanLyVe/XoaVe', function (req, res) {
    dbConn.query('DELETE FROM nodejsapi.datve WHERE maGhe = ?', [req.body.maGhe], function (error, results, fields) {
        if (error) throw error;
    })
});
...
app.delete('/api/DeleteTicketOfUser', function (req, res) {
    console.log(req.query.maGhe, "DELETE")
    dbConn.query('DELETE FROM nodejsapi.datve WHERE maGhe= ? ', [req.query.maGhe], async (error, results, fields) => {
        if (error) throw error;
        return res.send("Success");
    })
})
app.get('/api/QuanLyDatVe/LayDanhSachVeDuaTheoCumRap', function (req, res) {
    dbConn.query('SELECT * FROM lichchieuinsert JOIN phiminsertvalichchieuinsert ON lichchieuinsert.maLichChieu = phiminsertvalichchieuinsert.lichchieuinsert JOIN phiminsert ON phiminsert.maPhim = phiminsertvalichchieuinsert.phiminsert JOIN cumrapvalichchieuinsert ON lichchieuinsert.maLichChieu = cumrapvalichchieuinsert.lichchieuinsert JOIN cumrap ON cumrap.cid = cumrapvalichchieuinsert.cumrap JOIN datve ON datve.maLichChieu = lichchieuinsert.maLichChieu WHERE cumrap.maCumRap = ? ORDER BY ngayChieuGioChieu DESC', [req.query.maCumRap], async (error, results, fields) => {
        if (error) throw error;
        var danhSachVe = [];
        for (var i = 0; i < results.length; i++) {
            danhSachVe.push({
                "maGhe": results[i].maGhe,
                "maCumRap": results[i].cid,
                "maLichChieu": results[i].maLichChieu,
                "tenCumRap": results[i].tenCumRap,
                "tenRap": results[i].tenRap,
                "diaChi": results[i].diaChi,
...
app.get('/api/QuanLyDatVe/LayDanhSachVeDuaTheoLichChieu', function (req, res) {
    dbConn.query('SELECT * FROM lichchieuinsert JOIN phiminsertvalichchieuinsert ON lichchieuinsert.maLichChieu = phiminsertvalichchieuinsert.lichchieuinsert JOIN phiminsert ON phiminsert.maPhim = phiminsertvalichchieuinsert.phiminsert JOIN cumrapvalichchieuinsert ON lichchieuinsert.maLichChieu = cumrapvalichchieuinsert.lichchieuinsert JOIN cumrap ON cumrap.cid = cumrapvalichchieuinsert.cumrap JOIN datve ON datve.maLichChieu = lichchieuinsert.maLichChieu WHERE datve.maLichChieu = ? ORDER BY ngayChieuGioChieu DESC', [req.query.maLichChieu], async (error, results, fields) => {
        if (error) throw error;
        var danhSachVe = [];
        for (var i = 0; i < results.length; i++) {
            danhSachVe.push({
...
app.get('/api/QuanLyDatVe/LayDanhSachVeDuaTheoVe', function (req, res) {
    dbConn.query('SELECT * FROM lichchieuinsert JOIN phiminsertvalichchieuinsert ON lichchieuinsert.maLichChieu = phiminsertvalichchieuinsert.lichchieuinsert JOIN phiminsert ON phiminsert.maPhim = phiminsertvalichchieuinsert.phiminsert JOIN cumrapvalichchieuinsert ON lichchieuinsert.maLichChieu = cumrapvalichchieuinsert.lichchieuinsert JOIN cumrap ON cumrap.cid = cumrapvalichchieuinsert.cumrap JOIN datve ON datve.maLichChieu = lichchieuinsert.maLichChieu WHERE datve.maGhe = ? ORDER BY ngayChieuGioChieu DESC', [req.query.maGhe], async (error, results, fields) => {
        if (error) throw error;
        var danhSachVe = [];
        for (var i = 0; i < results.length; i++) {
            danhSachVe.push({
                "maGhe": results[i].maGhe,
                "maCumRap": results[i].maCumRap,
                "maLichChieu": results[i].maLichChieu,
                "tenCumRap": results[i].tenCumRap,
                "tenRap": results[i].tenRap,
                "diaChi": results[i].diaChi,
                ...
                dbConn.query(
                    ...
                    function (error, results2, fields) {
                        ...
                        for (const result2 of results2) {
                            ...
                            tenRap = result2.tenRap;
                            loaiGhe = result2.loaiGhe;
                            taiKhoanNguoiDat = result2.taiKhoanNguoiDat
                            ...
                            console.log("LOG DAT VE", email, req.body.maLichChieu, listVe, tenRap, tenCumRap, tenPhim, time, loaiGhe, taiKhoanNguoiDat);

                            var transporter = nodemailer.createTransport({
                                service: "gmail",
                                auth: {
                                    user: "giangnam.17122002@gmail.com",
                                    pass: "eognzbmuuispjodk",
                                },
                            });
                            ...
                            var mailOptions = {
                                ...
                                to: email,
                                subject: "Cinema cảm ơn bạn đã đặt vé tại Website của chúng tôi",
                                text: "Các thông tin về vé đặt:\n" +
                                    ...
                                    "Cảm ơn quý khách: " + taiKhoanNguoiDat
                            };
